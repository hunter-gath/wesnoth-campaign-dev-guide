...
(() => {
  ...
  const saved = localStorage.getItem("trow-theme");
  ...
  document.querySelectorAll("[data-theme-toggle]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      localStorage.setItem("trow-theme", next);
      btn.textContent = next === "dark" ? "Use light theme" : "Use dark theme";
    });
  });
  ...
  if (searchPage && Array.isArray(window.TROW_SEARCH_INDEX)) {
    ...
    const run = query => {
      ...
      const ranked = window.TROW_SEARCH_INDEX.map(item => {
      ...
      if (!ranked.length) {
        resultsNode.innerHTML = '<div class="search-empty">No matching pages. Try fewer terms or a campaign identifier such as <code>last_done</code>, <code>Jevyan</code>, or <code>GUARD_UNIT</code>.</div>';
        return;
      }
      resultsNode.innerHTML = ranked.slice(0, 80).map(item => `
        <article class="search-result">
          <h3><a href="${esc(item.url)}">${highlight(item.title, terms)}</a></h3>
          <div class="search-path">${esc(item.path)}</div>
